...
Bias_Variance_name = 'Bias_Variance_(eV)'  # lower bound of the bias charge change
...
def create_data_point_dict_steady(gate_amp, gate_func, bias_variance, bias_function, period, number_of_periods,
    ...
    dictionary = {
        ...
        Bias_Variance_name: bias_variance,
...
def exponentiate(gate_amp, gate_function, bias, n_set, time_array):
...
def current_calc(gate_amp, gate_func, bias, n_set, time_array, number_of_periods):
    dim = len(n_set)
    U = exponentiate(gate_amp, gate_func, bias, n_set, time_array)
