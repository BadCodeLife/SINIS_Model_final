...
if __name__ == '__main__':
    Existing_Data = cm.fetch_data(file_name, key)

    Gate_Amplitudes = input[cm.Gate_Amp_name]
    Leakages = input[cm.Leak_name]
    ...
    cm.calculate_points(points_to_calc, file_name, key, Existing_Data)
    Existing_Data = cm.fetch_data(file_name, key)
    ...
    for leak in Leakages:
        ...
        line_data_set[cm.Leak_name] = leak
        line_label = '%s$\,\Delta$' % leak
    ...
    Title = 'Basic 2 Variable plot for a %s transistor \n' % type \
            ...
            + 'Thermal Energy = %s $\Delta$, \n' % input[cm.Thermal_E_name] \
    ...
    ax.legend(title='Sub Gap Leakage ($\gamma$)')
    plt.show()
