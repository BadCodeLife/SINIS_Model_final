...
def vector_base(gate, limit):
    center = - int(round(gate))
    return sp.arange(center - limit, center + limit + 1)
...
def steady_state(gate, bias, limit):
    n_set = vector_base(gate, limit)
...
def current_point(point, calculated_points):
    ...
    current = steady_state(gate=point[Gate_name], bias=point[Bias_name], limit=point[States_name])
